...
@click.command()
@click.option("--repopath", default=".", help="path of directory containing data. Defaults to current directory")
@click.option("--version",default="latest", help="version of dataset. Eg : latest, latest-06...")
def main_plot(repopath,version):
    ...
    # if sys.argv() > 1:
    dfs= [pd.read_csv(
    '{repopath}/{site}/{file}'.format(repopath=repopath,site=site, file=version), skiprows=[0],names=names,
    sep = '\s+'
    ) for site in sites]
    set_dates(dfs)
